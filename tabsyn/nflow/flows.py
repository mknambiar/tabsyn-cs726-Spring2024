...
class NormalizingFlows(nn.Module):
    ...
    Args:
        n_features: int
            Number of features for transforming
        n_layers_hidden: int
    ...
    def __init__(
        ...
        base_dist = self._get_base_distribution()(shape = [n_features]).to(self.device)

        transform = self._create_transform(n_features).to(self.device)
    ...
    def generate(self, count: int) -> np.ndarray:
        return self(count).detach().numpy()
